...
public class Hand : MonoBehaviour
{
    ...
    private void physicsMove() 
    {
        ...
        var pwo = followTarget.position + positionOffset;
        var distance = Vector3.Distance(pwo, transform.position);
        body.velocity = (pwo - transform.position).normalized * (followSpeed * distance);
        
        var rwo = followTarget.rotation * Quaternion.Euler(rotationOffset);
        var quart = rwo * Quaternion.Inverse(body.rotation);
